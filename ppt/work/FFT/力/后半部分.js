...
sd.init(() => {
    for (let i = 1; i <= n; i++) {
        ...
        arrB.element(i).value(new sd.Math(arrB, `q_{${label}}`).fontSize(fontSize), R.center());
    }
    arrA.push("...");
    arrA.push(new sd.Math(arrA, "\\frac{1}{(n-j)^2}"));
    arrA.push(new sd.Math(arrA, "\\frac{1}{(n-j+1)^2}"));
