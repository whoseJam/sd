...
sd.init(() => {
    ...
    for (let i = 1; i <= n; i++) graph.element(i).childAs("label", new sd.Text(svg, "dis=inf"), R.aside(disLabel[i - 1]));
    links.forEach(link => {
        const x = link[0];
        const y = link[1];
        const v = link[2];
...
sd.main(async () => {
    await sd.pause();
    dis[1] = 0;
    graph.element(1).child("label").startAnimate().text("dis=0", { "dis=": "dis=" }).endAnimate();
    for (let i = 1; i <= n; i++) {
        let dis_ = Infinity;
        let u_ = 0;
        for (let u = 1; u <= n; u++) {
        ...
        for (let i = 0; i < out.length; i++) {
            ...
            if (dis[v] > out[i].intValue() && !vis[v]) {
                ...
                prt[v] = u_;
                const naive = graph.sourceId(out[i]) === String(u_);
                const line = new sd.Line(svg)
                    .source(naive ? out[i].source() : out[i].target())
                    .target(naive ? out[i].target() : out[i].source())
                    .stroke(C.deepSkyBlue)
                    .strokeWidth(3)
                    .startAnimate()
                    .pointStoT()
                    .endAnimate()
                    .arrow();
                graph.element(v).child("label").startAnimate().text(`dis=${out[i].intValue()}`, { "dis=": "dis=" }).endAnimate();
                lines.push(line);
            }
        }
        if (flag) await sd.pause();
        lines.forEach(line => line.startAnimate().fadeStoT().endAnimate().arrow(null).remove());
        await sd.pause();
