...
const svg = sd.svg();
const C = sd.color();
const colors = [C.red, C.red, C.blue, C.green, C.green, C.red, C.green, C.green];
...
async function onUpdateBucket(arr, j) {
    ...
    const id = colorTypes.indexOf(current.fill());
...
async function onCreateFirstBucket(arr, cx) {
    ...
    stk.cx(cx)
        ...
        .forEachElement((element, i) => {
            sd.Aside(element, new sd.Rect(svg).scale(0.2).color(colorTypes[i]), "lc");
        })
        .endAnimate();
    firstBucket = stk;
}
