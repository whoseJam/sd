...
sd.main(async () => {
    await status(S);
    await sd.pause();
    arr.startAnimate().color(r, C.grey).endAnimate();
    math.startAnimate()
    ...
    await sd.pause();
    const sol1 = new sd.Array(svg)
        .resize(n)
        .start(1)
        .cx(arr.cx())
        .y(bin.my() + 30);
    for (let i = sol1.start(); i <= sol1.end(); i++) sol1.value(i, 0);
    sol1.value(sol1.end() - r, 1)
        .opacity(0)
        .startAnimate()
        .opacity(1)
        .endAnimate();
    await sd.pause();
    sol1.startAnimate();
    for (let i = sol1.start(); i <= sol1.end(); i++) sol1.value(i, sol1.intValue(i) ^ 1);
    sol1.endAnimate();
    await sd.pause();
    sol1.startAnimate().opacity(0).endAnimate().remove();

    await sd.pause();
    const sol2 = new sd.Array(svg)
        .resize(n)
        .start(1)
        .cx(arr.cx())
        .y(bin.my() + 30);
    for (let i = sol2.start(); i <= sol2.end(); i++) sol2.value(i, 0);
    sol2.value(sol2.end() - r, 1)
        .opacity(0)
        .startAnimate()
        .opacity(1)
        .endAnimate();
