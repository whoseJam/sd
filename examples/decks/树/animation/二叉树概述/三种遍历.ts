...
sd.main(async () => {
  ...
  for (let i = 0; i < NODES.length; i++) {
    const d = 80 + i * 60;
    circles[i].startAnimate({ delay: d, duration: 260, easing: E.easeOut })
      .setOpacity(1).endAnimate();
    labels[i].startAnimate({ delay: d + 60, duration: 260, easing: E.easeOut })
      .setOpacity(1).endAnimate();
  }
  await sd.pause();
  ...
  preRow.keyText
    ...
    .setOpacity(1).setFill(PRE_COLOR).endAnimate();
  ...
  inRow.keyText
    .startAnimate({ duration: 280, easing: E.easeOut })
    .setOpacity(1).setFill(IN_COLOR).endAnimate();
  ...
  postRow.keyText
    .startAnimate({ duration: 280, easing: E.easeOut })
    .setOpacity(1).setFill(POST_COLOR).endAnimate();
