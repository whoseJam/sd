...
new sd.Math({
  targetNode: svg, text: `\\text{第一半 switches: } 2^{${N / 2}}`,
  cx: X0 + (N / 4) * SIZE, cy: row.top() + 36,
  fontSize: 13, fill: C.steelBlue,
});
new sd.Math({
  targetNode: svg, text: `\\text{第二半 switches: } 2^{${N / 2}}`,
  cx: X0 + (3 * N / 4) * SIZE, cy: row.top() + 36,
  fontSize: 13, fill: C.darkOrange,
});
