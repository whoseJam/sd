...
// len row sits inside t's group so it slides with t.
...
// Two underline brackets (drawn above s and above t respectively) that
// flash on each jump to show "this suffix on s equals this prefix on t".
...
function flashBracket(line: sd.Line, x1: number, x2: number, y: number) {
  // Snap geometry without animating the prior frame, then fade in.
...
sd.main(async () => {
  s.fadeIn({ delay: 0 });
  t.fadeIn({ delay: 140 });
  // len row sits inside t.group but needs its own fade because the group
  // itself isn't faded.
