...
export class FTable {
  ...
  constructor(opts: FTableOpts) {
    ...
    for (let j = 0; j < opts.rows; j++) {
      ...
      for (let i = 1; i <= opts.cols; i++) {
        ...
        cellRow.push(new sd.Rect({
          targetNode: this.group, x: cellX, y: cellY,
          width: opts.cellSize, height: opts.cellSize,
          fill: C.white, stroke: C.silver, strokeWidth: 1,
        }));
        glyphRow.push(new sd.Text({
          targetNode: this.group, text: "",
          cx: cellX + opts.cellSize / 2, cy: cellY + opts.cellSize / 2,
          fontSize: opts.cellSize * 0.4, fill: C.darkButtonGrey,
        }));
      }
    ...
    for (let j = 0; j < opts.rows; j++) {
      new sd.Math({
        targetNode: this.group, text: `2^{${j}}`,
        cx: opts.x - 24,
  ...
  paintCell(j: number, i: number, fill: sd.SDColor, stroke: sd.SDColor, opts?: { delay?: number; duration?: number }) {
    this.cellRects[j][i - 1]
      .startAnimate({ delay: opts?.delay ?? 0, duration: opts?.duration ?? 200, easing: E.easeOut })
      .setFill(fill).setStroke(stroke).endAnimate();
  }

  clearCell(j: number, i: number, opts?: { delay?: number; duration?: number }) {
    this.cellRects[j][i - 1]
      .startAnimate({ delay: opts?.delay ?? 0, duration: opts?.duration ?? 200, easing: E.easeOut })
      .setFill(C.white).setStroke(C.silver).endAnimate();
  }

  setValue(j: number, i: number, v: number | string, opts?: { delay?: number; duration?: number }) {
    this.glyphs[j][i - 1]
      .startAnimate({ delay: opts?.delay ?? 0, duration: opts?.duration ?? 200, easing: E.easeOut })
