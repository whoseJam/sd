...
const fRow = new NumRow({
  targetNode: svg, values: Fl.map((v) => v + 1), size: SIZE,
  x: X0, y: -30 - SIZE, label: "F",
});
...
sd.main(async () => {
  ...
  for (let i = 1; i <= N; i++) {
    ...
    for (let k = i; k <= right; k++) arr.paintCell(k, "#e8f5e9", C.darkGreen, { delay: base + (k - i) * 30, duration: 200 });
    fRow.paintCell(i, "#fdecd9", C.darkOrange, { delay: base + 60, duration: 200 });
    await sd.pause();
