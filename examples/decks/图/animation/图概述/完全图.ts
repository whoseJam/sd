...
for (let kIdx = 0; kIdx < Ks.length; kIdx++) {
  ...
  if (n === 1) {
    ...
    for (let i = 0; i < n; i++) {
      ...
      positions.push({ cx: cx0 + r * Math.cos(angle), cy: r * Math.sin(angle) });
    }
  }
  for (let i = 0; i < n; i++) {
    for (let j = i + 1; j < n; j++) {
      new sd.Line({
        ...
        x2: positions[j].cx, y2: positions[j].cy,
  ...
  for (const p of positions) {
    new sd.Circle({
      targetNode: svg, cx: p.cx, cy: p.cy, r: 5,
      fill: C.darkButtonGrey, stroke: C.darkButtonGrey,
    });
  }
  new sd.Math({
    targetNode: svg, text: `K^{${n}}`,
    cx: cx0, cy: 60, fontSize: 14, fill: C.darkButtonGrey,
  });
}
