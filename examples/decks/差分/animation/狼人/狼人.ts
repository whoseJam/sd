...
const c = [0, 0, 1, 2, 3, 2, 1, 0, -1, -2, -3, -2, -1, 0, 1, 2, 3, 2, 1, 0, 0, 0];
...
sd.main(async () => {
  ...
  for (let i = 1; i <= c.length; i++) {
    ...
    if (d1[i - 1] !== 0) d1R.paintCell(i, "#fdecd9", C.darkOrange, { delay: i * 30, duration: 180 });
  ...
  for (let i = 1; i <= c.length; i++) {
    ...
    if (d2[i - 1] !== 0) d2R.paintCell(i, "#dbeefd", C.steelBlue, { delay: i * 30, duration: 180 });
