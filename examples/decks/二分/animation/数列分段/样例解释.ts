...
sd.main(async () => {
  ...
  for (let k = 0; k < breaks.length; k++) {
    ...
    new sd.Line({
      ...
      x2: cx, y2: row.top() + 6,
      stroke: C.darkOrange, strokeWidth: 2.4,
      opacity: 0,
    }).startAnimate({ delay: k * 120, duration: 220, easing: E.easeOut }).setOpacity(1).endAnimate();
  }
  new sd.Math({
    targetNode: svg,
    text: `m = ${M}`,
    ...
    opacity: 0,
  }).startAnimate({ delay: 600, duration: 220, easing: E.easeOut }).setOpacity(1).endAnimate();
  await sd.pause();
});
