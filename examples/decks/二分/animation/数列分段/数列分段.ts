...
sd.main(async () => {
  row.fadeIn({ delay: 0 });
  mxLabel
    .startAnimate({ delay: 400, duration: 240, easing: E.easeOut })
    .setOpacity(1)
    .endAnimate();
  segLabel
    .startAnimate({ delay: 400, duration: 240, easing: E.easeOut })
  ...
  for (const limit of tries) {
    ...
    for (let k = 0; k < cuts.length; k++) {
      ...
      ln.startAnimate({ delay: k * 60, duration: 220, easing: E.easeOut })
        .setOpacity(1)
        .endAnimate();
      lines.push(ln);
    }
    segLabel
      ...
      .setText(`segments = ${cuts.length + 1}`)
      .endAnimate();
    await sd.pause();
    for (const ln of lines)
      ln.startAnimate({ duration: 180 }).setOpacity(0).endAnimate();
  }
});
