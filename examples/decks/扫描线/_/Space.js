...
function convertToWorld1(worldBox, viewBox, point) {
  const x =
    ((point[0] - viewBox.x) / (viewBox.width - 1)) * worldBox.width +
    worldBox.x;
  const y =
    worldBox.y +
    worldBox.height -
    ((point[1] - viewBox.y) / (viewBox.height - 1)) * worldBox.height;
  return [x, y];
}

function convertToWorld2(worldBox, viewBox, point) {
  const x =
    ((point[0] - viewBox.x + 0.5) / viewBox.width) * worldBox.width +
    worldBox.x;
  const y =
    worldBox.y +
    worldBox.height -
    ((point[1] - viewBox.y + 0.5) / viewBox.height) * worldBox.height;
  return [x, y];
}
...
export class Space extends sd.SDNode {
  constructor(parent) {
    ...
    this.effect("layoutElement", () => {
      const convert =
        this.vars.tickType === "atPoint" ? convertToWorld1 : convertToWorld2;
      this.vars.elements.forEach((element) => {
        if (element.rect) {
          const mn = convert(this.worldBox(), this.viewBox(), [
            element.x,
            element.y,
          ]);
          ...
          const pos = convert(this.worldBox(), this.viewBox(), [
            element.x,
            element.y,
          ]);
    ...
    this.childAs(new Axis(this).direction("horizontal"), (parent, child) => {
      ...
      child.tickCount(
        parent.tickType() === "atPoint"
          ? parent.vars.viewBox.width
          : parent.vars.viewBox.width + 1,
      );
    });
    this.childAs(new Axis(this).direction("vertical"), (parent, child) => {
      ...
      child.tickCount(
        parent.tickType() === "atPoint"
          ? parent.vars.viewBox.height
          : parent.vars.viewBox.height + 1,
      );
    });
  }
  x(x) {
  ...
  globalX(x) {
    if (this.tickType() === "atPoint")
      return convertToWorld1(this.worldBox(), this.viewBox(), [x, 0])[0];
    return convertToWorld2(this.worldBox(), this.viewBox(), [x, 0])[0];
  }
  globalY(y) {
    if (this.tickType() === "atPoint")
      return convertToWorld1(this.worldBox(), this.viewBox(), [0, y])[1];
    return convertToWorld2(this.worldBox(), this.viewBox(), [0, y])[1];
  }
  width(width) {
    if (arguments.length === 0) {
      if (this.tickType() === "atPoint")
        return this.identityWidth() * (this.vars.viewBox.width - 1);
  ...
  height(height) {
    if (arguments.length === 0) {
      if (this.tickType() === "atPoint")
        return this.identityHeight() * (this.vars.viewBox.height - 1);
