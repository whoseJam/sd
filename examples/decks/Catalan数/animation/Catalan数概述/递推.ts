...
const row = new NumRow({
  targetNode: svg, values: new Array(TOTAL).fill(" "), size: SIZE,
  x: X0, y: 0,
});
...
sd.main(async () => {
  ...
  row.setValue(1, "A");
  row.paintCell(2 * split, "#dbeefd", C.steelBlue, { duration: 200, delay: 80 });
  ...
  new sd.Math({
    targetNode: svg, text: `H_{${split - 1}}`,
    cx: (row.cellLeft(2) + row.cellRight(2 * split - 1)) / 2, cy: row.top() + 22,
    fontSize: 14, fill: C.darkOrange, opacity: 0,
  }).startAnimate({ duration: 280, easing: E.easeOut }).setOpacity(1).endAnimate();
  await sd.pause();
  for (let i = 2 * split + 1; i <= TOTAL; i++) {
    row.paintCell(i, "#e8f5e9", C.darkGreen, { delay: (i - 2 * split) * 30, duration: 200 });
  }
  new sd.Math({
    targetNode: svg, text: `H_{${N - split}}`,
    cx: (row.cellLeft(2 * split + 1) + row.cellRight(TOTAL)) / 2, cy: row.top() + 22,
    fontSize: 14, fill: C.darkGreen, opacity: 0,
  }).startAnimate({ duration: 280, easing: E.easeOut }).setOpacity(1).endAnimate();
  await sd.pause();
});
