...
sd.main(async () => {
    ...
    const bg = new sd.Rect({ x: 0, y: 0, width: 1200, height: 600, fill: "#080808" });
    svg.appendChild(bg);

    for (let i = 0; i < 600; i += 50) {
        svg.appendChild(new sd.Line({ x1: 0, y1: i, x2: 1200, y2: i, stroke: "#151515", strokeWidth: 2 }));
        for (let j = 0; j < 1200; j += 100) {
            let off = (i / 50) % 2 === 0 ? 0 : 50;
            svg.appendChild(
                new sd.Line({ x1: j + off, y1: i, x2: j + off, y2: i + 50, stroke: "#151515", strokeWidth: 2 })
            );
        }
    }

    const tStyle = { text: "你好，世界", fontSize: 90, fontFamily: "Arial", fontWeight: "bold" };

    const tMain = new sd.Text({ ...tStyle, fill: "#fff", stroke: "#ff00ff", strokeWidth: 2 });
    const tGlow = new sd.Text({ ...tStyle, fill: "none", stroke: "#d600ff", strokeWidth: 12, strokeOpacity: 0.2 });
