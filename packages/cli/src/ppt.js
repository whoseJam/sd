  ...
  walk(source, (path) => {
    ...
    if (!eventListener[suffix] || !eventListener[suffix].onAdd) {
      console.log(
        colors(
          "red",
          `[Error] No 'onAdd' handler defined for file with extension ${suffix} at ${path}.`,
        ),
      );
      return;
    }
    eventListener[suffix].onAdd(
      pathToOriginFile(path),
      pathToTargetFolder(path),
    );
  });
    ...
    watcher.on("change", function (path) {
      ...
      if (!eventListener[suffix] || !eventListener[suffix].onChange) {
        console.log(
          colors(
            "red",
            `[Error] No 'onChange' handler defined for file with extension ${suffix} at ${path}.`,
          ),
        );
        return;
      }
    ...
    watcher.on("unlink", function (path) {
      ...
      if (!eventListener[suffix] || !eventListener[suffix].onUnlink) {
        console.log(
          colors(
            "red",
            `[Error] No 'onUnlink' handler defined for file with extension ${suffix} at ${path}.`,
          ),
        );
        return;
      }
