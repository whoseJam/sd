...
export class SDAnimationElement extends HTMLElement {
    static get observedAttributes() {
        return ["src", "viewbox", "viewbox-delta", "rate", "description", "max-frame", "args"];
    }
    ...
    _sendInit() {
        ...
        if (vb) {
            const final = delta
                ? { x: vb.x + delta.x, y: vb.y + delta.y, width: vb.width + delta.width, height: vb.height + delta.height }
                : vb;
            this._post("SetViewBox", [final.x, final.y, final.width, final.height, rate]);
        }
        this._post("Message", ["IFRAME_ID", this._frameId]);
        this._post("Message", ["IFRAME_URL", src]);
        if (argsAttr) {
            try { this._post("Message", ["IFRAME_ARGS", JSON.parse(argsAttr)]); } catch {}
        }
...
if (typeof window !== "undefined" && window.customElements) {
    if (!window.customElements.get("sd-animation")) {
        ...
        console.warn("[@sd/element] <sd-animation> already registered; this script load is a no-op.");
    }
}
