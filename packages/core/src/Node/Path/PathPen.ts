...
export class PathPen {
  ...
  Cubic(x1: any, y1: any, x2?: any, y2?: number, x?: number, y?: number): this {
    ...
    [x1, y1, x2, y2, x, y].forEach((v, i) =>
      Check.validateNumber(+v, "PathPen.Cubic", i + 1),
    );
  ...
  cubic(
    ...
    [dx1, dy1, dx2, dy2, dx, dy].forEach((v, i) =>
      Check.validateNumber(+v, "PathPen.cubic", i + 1),
    );
  ...
  Quad(x1: any, y1: any, x?: number, y?: number): this {
    ...
    [x1, y1, x, y].forEach((v, i) =>
      Check.validateNumber(+v, "PathPen.Quad", i + 1),
    );
  ...
  Arc(
    ...
    [rx, ry, rotation, x, y].forEach((v, i) =>
      Check.validateNumber(+v, "PathPen.Arc", i + 1),
    );
    if (large !== 0 && large !== 1)
      throw new Error(`LargeArcFlag must be 0 or 1, got ${large}`);
    if (sweep !== 0 && sweep !== 1)
      throw new Error(`SweepFlag must be 0 or 1, got ${sweep}`);
  ...
  arc(
    ...
    [rx, ry, rotation, dx, dy].forEach((v, i) =>
      Check.validateNumber(+v, "PathPen.arc", i + 1),
    );
    if (large !== 0 && large !== 1)
      throw new Error(`LargeArcFlag must be 0 or 1, got ${large}`);
