...
import type { SubtextView, TextView } from "@/node/text/text-engine/text-view";

import { Action } from "@/animate/action";
import { Animate as A } from "@/animate/animate";
import { lazyInterp } from "@/animate/interp";
import { getTextPaths, getTextPaths2 } from "@/node/text/text-engine/path";
...
export function typewritterPostProcess(text: Text, targetLayer: RenderNode) {
  return lazyInterp(function (
    ...
    const sourcePaths = getTextPaths2(
      text,
      l,
      A.getAttribute(text, "text", r, text.getText()),
    );
      ...
      new Action(
        ...
        function (t: number) {
          ...
          for (let i = 0; i < characters.length; i++) {
            const k = this.reverse
              ? (characters.length - i - 0.5) * time
              : (i + 0.5) * time;
