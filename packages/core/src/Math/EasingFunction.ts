type SDEasingFunctionLiteral =
  ...
  | "bounce-in-out"
  | "bounceInOut";
export type SDEasingFunction =
  | SDEasingFunctionLiteral
  | ((t: number) => number);
...
export class EasingFunction {
  ...
  static bounceInOut(t: number) {
    return t < 0.5
      ? EasingFunction.bounceIn(t * 2) * 0.5
      : EasingFunction.bounceOut(t * 2 - 1) * 0.5 + 0.5;
  }
  ...
  static toEasingFunction(easing: SDEasingFunction): (t: number) => number {
    ...
    if (easing.includes("-"))
      easing = easing.replace(/-([a-z])/g, (match, letter) =>
        letter.toUpperCase(),
      ) as SDEasingFunctionLiteral;
    return this[easing];
  }
}
