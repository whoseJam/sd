...
sd.main(async () => {
    ...
    sum.startAnimate()
        ...
        .endAnimate();
    await sd.pause();
    text.element(2).startAnimate().subtextColor("isPrime(i)", C.green).endAnimate();
    await sd.pause();
    sum.startAnimate()
        .text("\\sum_{i=1}^{n}i[i\\; is\\; prime]", [
            ["", "[i\\; is\\; prime]"],
            ["i=1", "i=1"],
            ["n", "n"],
            ["i", "i"],
        ])
        .subtextColor("[i\\; is\\; prime]", C.green)
        .endAnimate();
    await sd.pause();
    sum.startAnimate()
        .text("\\sum_{\\begin{aligned}1\\le & i\\le n\\\\i\\; is \\; &prime \\end{aligned} }i", [
            //
            ["i=1", "\\begin{aligned}1\\le & i\\le n\\end{aligned}"],
            { source: "n", target: { subtext: "n", i: 0 } },
            ["[i\\; is\\; prime]", "\\begin{aligned}i\\; is \\; &prime \\end{aligned}"],
            ["i", "i"],
        ])
        .cy(text.cy())
        .endAnimate();
