...
const data1 = sd.make2d(n, n);
links.forEach(link => (data1[link[1] - 1][link[0] - 1] = 1));
const data2 = [[1], [0], [0], [0], [0], [0]];
const data3 = [["?"], ["?"], ["?"], ["?"], ["?"], ["?"]];
...
sd.init(() => {
    ...
    graph.cx((matrix1.x() + matrix3.mx()) / 2).y(matrix1.my() + 50);
...
sd.main(async () => {
    ...
    for (let n = 1; n <= 5; n++) {
        ...
        graph.startAnimate();
        for (let i = 0; i < data2_.length; i++) graph.color(i + 1, data2_[i][0] ? C.green : C.white);
        graph.endAnimate();
    }
});
