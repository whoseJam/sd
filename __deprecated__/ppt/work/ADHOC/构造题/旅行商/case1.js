...
sd.main(async () => {
    await sd.pause();
    sd.Link(graph.vertex(), graph.S()).stroke(C.red).startAnimate().pointStoT().endAnimate();
    sd.Link(graph.vertex(), graph.T()).stroke(C.red).startAnimate().pointStoT().endAnimate();
});
