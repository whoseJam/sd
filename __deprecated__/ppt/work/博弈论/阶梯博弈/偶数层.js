...
sd.init(() => {
    for (let i = 0; i < data.length; i++) {
        const rect = new sd.Rect(svg).height(i * 60).width(W).x(i * W + W).my(0);
        const arr = new sd.ValueArray(svg).elementWidth(22).elementHeight(22).my(rect.y() - 2);
        arrs.push(arr);
        for (let j = 0; j < data[i]; j++) {
            let curIndex = i;
            const circ = new sd.Circle(svg).r(10).color(C.grey);
            arr.push(circ.onClick(() => {
                sd.inter(async () => {
