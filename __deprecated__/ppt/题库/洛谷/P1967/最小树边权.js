...
sd.main(async () => {
    for (let i = 2; i <= 4; i <<= 1) {
        ...
        await sd.pause();
        const links = [];
        links.push(link(n, mid, C.red, `${n}到${mid}的最小树边权`));
        await sd.pause();
        links.push(link(mid, final, C.orange, `${mid}到${final}的最小树边权`));
        await sd.pause();
        links.push(link(n, final, C.black, `${n}到${final}的最小树边权`, -0.5, "mx"));
        await sd.pause();
